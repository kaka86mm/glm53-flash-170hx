...
import os
PROMPTS = ["用Python写一个快速排序，并解释复杂度。", "Explain the difference between TCP and UDP in detail.", "写一段关于秋天的散文", "Solve step by step: what is 847 times 23?", "介绍一下光合作用的过程。", "Write a bash script that backs up a directory with rotation.", "列举十个中国城市并各写一句话介绍。", "What are the main causes of the French Revolution?"]
tot = [0]*N; stop = time.time() + DUR
def worker(i):
    j = i
...
print(f"steady N={N} dur={dt:.0f}s tokens={sum(tot)} aggregate={sum(tot)/dt:.1f} t/s per-stream={sum(tot)/dt/N:.1f}")
