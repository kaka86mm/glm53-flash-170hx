...
## 🧪 patch-013 两轮实验终审（2026-09-19 09:4x UTC）：CPU 层封存

用满池逐出回放探针（60k 标记前缀 → 40 填充逐出 → 密码检索）做的对照实验：

| 配置 | CPU 层行为 | 回放正确性 | 判定 |
|---|---|---|---|
| v31（全组卸载） | 回载 1.05GB，clamp×2 | **0/3 密码**（12288 预算烧光，content+reasoning 双空），短请求金丝雀完美 | 污染，绑定长前缀回载 |
| v32（MLA-only 排除） | **存储 0 字节**（store 路径结构性依赖完整组集） | 等价于无 CPU 层 | 完全惰性 |

**终审**：全组=污染、MLA-only=无用 ⇒ 128G CPU KV 层在本 fork 世代不可得。
正解 = 正确实现 mamba/draft-SWA chunk 往返语义（上游 #52773/#52774 领地，
1-2 天 PR 级工程）。patch-013（env 门控 VLLM_OFFLOAD_MLA_ONLY）留作实验记录。

**生产终态：v30**（v27 + patch-012 paired-fg，无 CPU 层）。
