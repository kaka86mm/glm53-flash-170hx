...
docker run -d --name glm-exl3 --runtime=nvidia --gpus all --ipc=host \
  ...
  -e VLLM_EXL3_H2D_STAGGER_S=12 -e VLLM_PRETEND_NO_DEEP_GEMM=1 -e PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True -e VLLM_EXL3_OUTER_GRAPH=1 \
  ...
  --max-model-len 524288 --max-num-seqs 6 \
  ...
  --linear-backend marlin --async-scheduling --jit-monitor-mode warn \
